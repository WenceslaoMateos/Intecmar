      ...
      <div className="container mx-auto px-6 max-w-7xl">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-6">
          ...
          <div className="flex gap-2 flex-wrap">
            <button className="px-4 py-2 bg-brand-teal text-white rounded-full text-sm font-semibold transition shadow-sm hover:bg-brand-dark">Todo</button>
            <button className="px-4 py-2 bg-white text-gray-600 border border-gray-200 hover:bg-gray-100 rounded-full text-sm font-semibold transition">Noticias</button>
            <button className="px-4 py-2 bg-white text-gray-600 border border-gray-200 hover:bg-gray-100 rounded-full text-sm font-semibold transition">Convocatorias</button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          {noticiasMock.map((noticia) => (
            <article key={noticia.id} className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition duration-300 flex flex-col h-full group">
              <div className="h-48 bg-gray-200 relative overflow-hidden">
                <img src={noticia.imagen} alt={noticia.titulo} className="w-full h-full object-cover group-hover:scale-105 transition duration-500" />
              ...
              <div className="p-6 flex-grow flex flex-col">
                ...
                {/* título con link clickeable */}
                <Link href={`/novedades/${noticia.id}`} className="text-xl font-bold text-gray-800 mb-3 hover:text-brand-teal transition">
                  {noticia.titulo}
                </Link>
                ...
                {/* ruta dinámica */}
                <Link href={`/novedades/${noticia.id}`} className="mt-auto text-brand-teal font-bold text-sm hover:underline flex items-center group-hover:text-brand-dark">
                  Leer más <i className="fas fa-arrow-right ml-1 transform group-hover:translate-x-1 transition"></i>
                </Link>
              </div>
            </article>
          ))}
