...
export default function EditarPerfilPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    email: '' // El email suele venir del back pero se deshabilita para edición
  });
  ...
  // =========================================================================
  // 3. FETCH DE DATOS (CATÁLOGOS Y PERFIL DEL USUARIO)
  // =========================================================================
  useEffect(() => {
    setMounted(true);
  }, []);
      ...
      try {
        ...
        if (userRes.data) {
          ...
          setFormData({
            ...
            tipoDocumento: userData.typeDocument || '',
            numeroDocumento: userData.numberDocument || '',
            cuil: userData.cuilCuit || '',
            ...
            empresaEnMarcha: userData.empresaEnMarcha || '',
            fechaInicioEmpresa: userData.fechaInicioEmpresa || '',
            experienciaPrevia: userData.experienciaPrevia || '',
            motivacionEmprender: userData.motivacionEmprender || '',
            institucionReferente: userData.institucionReferente || '',
            otraInstitucion: userData.otraInstitucion || '',
            serviciosOfrecidos: userData.serviciosOfrecidos || '',
            serviciosAportados: userData.serviciosAportados || '',
            motivacionesParticipar: userData.motivacionesParticipar || [],
            nivelEstudios: userData.nivelEstudios || '',
            tituloAlcanzado: userData.tituloAlcanzado || '',
            areaConocimiento: userData.areaConocimiento || '',
            cursos: userData.cursos || [],
            vinculaciones: userData.vinculaciones || [],
        ...
        setError("Hubo un problema al cargar tu información.");
      } finally {
  ...
  useEffect(() => {
    ...
    // Evitamos blanquear la provincia si recién estamos cargando los datos iniciales
    if (!isLoadingData) {
        setCountiesDB([]);
        setCitiesDB([]);
    }
  }, [formData.pais, isLoadingData]); 
  ...
  const handleSave = async (e: React.SyntheticEvent) => {
    ...
    if (formData.roles.length === 0) {
      setError('Debes tener al menos un rol en la sección de Perfil.');
      setOpenSection(3);
      return;
    }
    ...
    try {
      ...
      // El correo o CUIL pueden no enviarse si son de solo lectura, o enviarse igual.
      ...
      // NOTA: Ajustá la ruta '/users/me' y el verbo HTTP según tu backend (PUT o PATCH)
      await api.put('/users/me', submitData);
      ...
      setError(backendMessage || 'Error al guardar los cambios.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  if (isLoadingData) {
    return (
      <div className="flex justify-center items-center h-screen bg-gray-100">
        <div className="text-brand-teal text-xl font-bold flex flex-col items-center">
        ...
        <div className="max-w-4xl">
          ...
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <section className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
              <button type="button" onClick={() => toggleSection(1)} className="w-full flex justify-between items-center p-5 bg-gray-50 hover:bg-gray-100 transition-colors">
                <h3 className="text-lg font-bold text-brand-teal font-heading"><i className="fas fa-user text-sm mr-2"></i> Información Básica</h3>
              ...
              <div className={`${openSection === 1 ? 'block' : 'hidden'} p-6 border-t border-gray-200 animate-fade-in`}>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <input type="text" name="numeroDocumento" value={formData.numeroDocumento} onChange={handleChange} className="w-full px-3 py-2 text-sm border rounded bg-white" />
                  </div>

                  <div>
                    <label className="block text-gray-500 text-xs font-bold mb-1">CUIL/T (No editable)</label>
                    <input type="text" name="cuil" value={formData.cuil} disabled className="w-full px-3 py-2 text-sm border rounded bg-gray-100 text-gray-500 cursor-not-allowed" />
                  </div>
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-gray-500 text-xs font-bold mb-1">Correo Electrónico (No editable)</label>
                    <input type="email" value={formData.email} disabled className="w-full px-3 py-2 text-sm border rounded bg-gray-100 text-gray-500 cursor-not-allowed" />
                  </div>
                </div>
              </div>
            </section>
            ...
            {/* --- SECCIÓN 2: UBICACIÓN --- */}
            <section className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
              <button type="button" onClick={() => toggleSection(2)} className="w-full flex justify-between items-center p-5 bg-gray-50 hover:bg-gray-100 transition-colors">
                <h3 className="text-lg font-bold text-brand-teal font-heading"><i className="fas fa-map-marker-alt text-sm mr-2"></i> Ubicación</h3>
              ...
              <div className={`${openSection === 2 ? 'block' : 'hidden'} p-6 border-t border-gray-200 animate-fade-in`}>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <div className="lg:col-span-2">
                    ...
                    {mounted && (
                      <Select
                        instanceId="select-pais-edit"
                        options={countriesDB.map(c => ({ value: c.id_country, label: c.name }))}
                        value={countriesDB.map(c => ({ value: c.id_country, label: c.name })).find(c => c.value === formData.pais) || null}
                  ...
                  <div className="lg:col-span-2">
                    ...
                    {mounted && (
                      <Select
                        instanceId="select-provincia-edit"
                        options={provincesDB.map(p => ({ value: p.id_province, label: p.name }))}
                        value={provincesDB.map(p => ({ value: p.id_province, label: p.name })).find(p => p.value === formData.provincia) || null}
                  ...
                  <div className="lg:col-span-2">
                    ...
                    {mounted && (
                      <Select
                        instanceId="select-partido-edit"
                        options={countiesDB.map(c => ({ value: c.id_county, label: c.name }))}
                        value={countiesDB.map(c => ({ value: c.id_county, label: c.name })).find(c => c.value === formData.partido) || null}
                  ...
                  <div className="lg:col-span-2">
                    ...
                    {mounted && (
                      <Select
                        instanceId="select-localidad-edit"
                        options={citiesDB.map(c => ({ value: c.id_city, label: c.name }))}
                        value={citiesDB.map(c => ({ value: c.id_city, label: c.name })).find(c => c.value === formData.localidad) || null}
            ...
            <section className="bg-white border border-blue-200 rounded-xl overflow-hidden shadow-sm">
              <button type="button" onClick={() => toggleSection(3)} className="w-full flex justify-between items-center p-5 bg-blue-50/50 hover:bg-blue-100/50 transition-colors">
                <h3 className="text-lg font-bold text-brand-teal font-heading"><i className="fas fa-briefcase text-sm mr-2"></i> Perfil</h3>
              ...
              <div className={`${openSection === 3 ? 'block' : 'hidden'} p-6 border-t border-blue-100 animate-fade-in bg-blue-50/10`}>
                ...
                {/* Opciones extra según el rol seleccionado (Exacto como en registro) */}
            ...
            {/* --- SECCIÓN 4: FORMACIÓN --- */}
            <section className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
              <button type="button" onClick={() => toggleSection(4)} className="w-full flex justify-between items-center p-5 bg-gray-50 hover:bg-gray-100 transition-colors">
                <h3 className="text-lg font-bold text-brand-teal font-heading"><i className="fas fa-graduation-cap text-sm mr-2"></i> Formación y experiencia profesional</h3>
              ...
              <div className={`${openSection === 4 ? 'block' : 'hidden'} p-6 border-t border-gray-200 animate-fade-in`}>
                ...
                <div className="mt-6 border-t border-gray-100 pt-4">
                  <label className="block text-gray-700 text-xs font-bold mb-1">Actualizar Curriculum Vitae (Opcional)</label>
                  <p className="text-xs text-gray-500 mb-2">Subir un nuevo archivo reemplazará el actual. Formato: PDF. Máx. 10 MB.</p>
                  <input 
                    type="file" 
                    accept=".pdf" 
            ...
            <section className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
              <button type="button" onClick={() => toggleSection(5)} className="w-full flex justify-between items-center p-5 bg-gray-50 hover:bg-gray-100 transition-colors">
                <h3 className="text-lg font-bold text-brand-teal font-heading"><i className="fas fa-handshake text-sm mr-2"></i> Vinculación con la Red INTECMAR</h3>
            ...
            {/* --- SECCIÓN 6: CONTACTO --- */}
            <section className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
              <button type="button" onClick={() => toggleSection(6)} className="w-full flex justify-between items-center p-5 bg-gray-50 hover:bg-gray-100 transition-colors">
                <h3 className="text-lg font-bold text-brand-teal font-heading"><i className="fas fa-address-book text-sm mr-2"></i> Contacto y Redes</h3>
            ...
            <div className="pt-6 mt-4 border-t border-gray-200 flex justify-end gap-4">
              <Link href="/perfil" className="px-6 py-2.5 bg-white border border-gray-300 text-gray-700 rounded-lg font-bold text-sm hover:bg-gray-50 transition-colors">
                Descartar
              </Link>
              <button type="submit" disabled={isLoading} className="px-8 py-2.5 bg-brand-teal text-white rounded-lg font-bold text-sm hover:bg-brand-dark transition-colors shadow-md disabled:opacity-70 flex items-center">
                {isLoading ? <><i className="fas fa-circle-notch fa-spin mr-2"></i> Guardando...</> : 'Guardar Cambios'}
              </button>
            </div>
