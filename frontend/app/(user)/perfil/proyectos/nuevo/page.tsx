...
export default function ProyectoNuevoPage() {
  // =========================================================================
  // 1. ESTADO UNIFICADO DEL FORMULARIO (Basado en el PDF)
  // =========================================================================
  const [formData, setFormData] = useState({
    ...
    // --- Sec 5: Innovación, Validaciones y Avance ---
  ...
  // Estado separado para archivos (PDF, JPG)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(true);
      setTimeout(() => {
        window.location.href = '/perfil'; // Redirige al perfil después de guardar (usando vanilla JS para compatibilidad)
      }, 2000);
    } catch (err) {
  ...
  return (
    <div className="bg-gray-100 min-h-screen py-6 px-4 flex justify-center items-center fade-in">
      <div className="bg-white w-full max-w-7xl rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row h-[90vh]">
        ...
        {/* ================= PANEL IZQUIERDO ================= */}
        <div className="w-full md:w-1/4 bg-brand-dark p-8 md:p-10 text-white flex flex-col relative overflow-hidden hidden md:flex shrink-0">
          <div className="absolute top-0 left-0 w-full h-full opacity-10 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')]"></div>
          
          <div className="relative z-10 mb-8">
            <a href="/perfil" className="text-gray-300 hover:text-white transition flex items-center gap-2 text-xs font-bold mb-8 opacity-80 hover:opacity-100">
              <i className="fas fa-arrow-left"></i> Volver al perfil
            </a>
            <h3 className="text-3xl font-bold mb-4 font-heading leading-tight">Registrar<br/>Proyecto</h3>
            <p className="text-gray-300 text-sm leading-relaxed">
              Completá la información para sumar tu iniciativa al ecosistema de Intecmar y acceder a beneficios exclusivos.
            </p>
          </div>
          ...
          <div className="relative z-10 flex-grow">
            <h4 className="text-sm uppercase tracking-wider font-bold mb-4 text-gray-400">Progreso del formulario</h4>
            <ul className="space-y-3 text-xs font-medium">
              {menuItems.map((item) => (
                <li 
                  key={item.id} 
                  className={`flex items-center cursor-pointer transition-all duration-200 ${openSection === item.id ? 'text-brand-teal font-bold scale-105 origin-left' : 'text-gray-400 hover:text-gray-200'}`}
        ...
        <div className="w-full md:w-3/4 p-6 md:p-10 overflow-y-auto bg-gray-50/50 custom-scrollbar relative">
          ...
          {success && <div className="bg-green-50 border-l-4 border-brand-success text-green-700 p-3 rounded mb-6 text-sm"><i className="fas fa-check-circle mr-2"></i> ¡Proyecto guardado exitosamente! Redirigiendo...</div>}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <section className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
              <button type="button" onClick={() => toggleSection(1)} className="w-full flex justify-between items-center p-4 bg-gray-50 hover:bg-gray-100 transition-colors">
                <h3 className="text-lg font-bold text-brand-teal font-heading"><i className="fas fa-info-circle mr-2"></i> 1. Información General y Ubicación</h3>
              ...
              <div className={`${openSection === 1 ? 'block' : 'hidden'} p-6 border-t border-gray-200 animate-fade-in`}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <input type="text" name="nombreProyecto" value={formData.nombreProyecto} onChange={handleChange} required className="w-full px-3 py-2 text-sm border rounded bg-white" />
                  </div>
                  <div className="md:col-span-2">
                    ...
                    <input type="text" name="nombreFantasia" value={formData.nombreFantasia} onChange={handleChange} className="w-full px-3 py-2 text-sm border rounded bg-white" />
                  </div>

                  {/* Contactos */}
                  <div>
                    <label className="block text-gray-700 text-xs font-bold mb-1">E-mail del proyecto *</label>
                    <input type="email" name="emailProyecto" value={formData.emailProyecto} onChange={handleChange} required className="w-full px-3 py-2 text-sm border rounded bg-white" />
                  </div>
                  <div>
                    <label className="block text-gray-700 text-xs font-bold mb-1">Página Web</label>
                    <input type="text" name="web" value={formData.web} onChange={handleChange} placeholder="https://" className="w-full px-3 py-2 text-sm border rounded bg-white" />
                  </div>
                  <div>
                    <label className="block text-gray-700 text-xs font-bold mb-1">Instagram</label>
                    <input type="text" name="instagram" value={formData.instagram} onChange={handleChange} placeholder="@miproyecto" className="w-full px-3 py-2 text-sm border rounded bg-white" />
                  </div>
                  <div>
                    <label className="block text-gray-700 text-xs font-bold mb-1">Año de inicio *</label>
                    <input type="number" name="anioInicio" value={formData.anioInicio} onChange={handleChange} required placeholder="Ej: 2023" className="w-full px-3 py-2 text-sm border rounded bg-white" />
                  </div>
                  ...
                  <div className="md:col-span-2 mt-4">
                    ...
                    <textarea name="propuestaValor" value={formData.propuestaValor} onChange={handleChange} required rows={3} className="w-full px-3 py-2 text-sm border rounded bg-white resize-none"></textarea>
                  </div>
                  <div className="md:col-span-2">
                    ...
                    <textarea name="solucionPropuesta" value={formData.solucionPropuesta} onChange={handleChange} required rows={3} className="w-full px-3 py-2 text-sm border rounded bg-white resize-none"></textarea>
                  </div>
                  ...
                  <div className="md:col-span-2 mt-2 border-t border-gray-100 pt-4">
                    ...
                    <select name="categoria" value={formData.categoria} onChange={handleChange} required className="w-full px-3 py-2 text-sm border rounded bg-white">
                  ...
                  <div className="md:col-span-2 mt-2 border-t border-gray-100 pt-4">
                    ...
                    <div className="flex gap-4">
                      <label className="flex items-center text-sm"><input type="radio" name="tipoUbicacion" value="Física" checked={formData.tipoUbicacion === 'Física'} onChange={handleChange} className="mr-2 text-brand-teal" /> Física</label>
                      <label className="flex items-center text-sm"><input type="radio" name="tipoUbicacion" value="Virtual" checked={formData.tipoUbicacion === 'Virtual'} onChange={handleChange} className="mr-2 text-brand-teal" /> Virtual</label>
                    </div>
                  </div>
                  ...
                  {formData.tipoUbicacion === 'Física' && (
                    <>
                      <div>
                        ...
                        <input type="text" name="domicilio" value={formData.domicilio} onChange={handleChange} required className="w-full px-3 py-2 text-sm border rounded bg-white" />
                      </div>
                      <div>
                        <label className="block text-gray-700 text-xs font-bold mb-1">Provincia *</label>
                        {/* Nota: En la fase 2 reemplazaremos estos inputs básicos por los React-Select encadenados */}
                        <input type="text" name="provincia" value={formData.provincia} onChange={handleChange} required className="w-full px-3 py-2 text-sm border rounded bg-white" placeholder="Ej: Buenos Aires" />
                      </div>
                    </>
                  )}
                </div>
            ...
            <section className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
              <button type="button" onClick={() => toggleSection(2)} className="w-full flex justify-between items-center p-4 bg-gray-50 hover:bg-gray-100 transition-colors">
                <h3 className="text-lg font-bold text-brand-teal font-heading"><i className="fas fa-file-contract mr-2"></i> 2. Formalización y Representante Legal</h3>
                ...
                <div className="mb-6 bg-blue-50/50 p-4 rounded-lg border border-blue-100">
                  ...
                  <div className="flex gap-6">
                    <label className="flex items-center cursor-pointer text-sm font-medium text-gray-700">
                      <input type="radio" name="esFormalizado" value="SI" checked={formData.esFormalizado === 'SI'} onChange={handleChange} className="mr-2 w-4 h-4 text-brand-teal focus:ring-brand-teal" />
                      SÍ, ES UNA EMPRESA
                    </label>
                    <label className="flex items-center cursor-pointer text-sm font-medium text-gray-700">
                      <input type="radio" name="esFormalizado" value="NO" checked={formData.esFormalizado === 'NO'} onChange={handleChange} className="mr-2 w-4 h-4 text-brand-teal focus:ring-brand-teal" />
                      NO, ES UN PROYECTO
                    </label>
                  </div>
                </div>
                ...
                {formData.esFormalizado === 'SI' && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-fade-in border-l-2 border-brand-teal pl-4 ml-2">
                    <div>
                      ...
                      <input type="number" name="anioFormalizacion" value={formData.anioFormalizacion} onChange={handleChange} required className="w-full px-3 py-2 text-sm border rounded bg-white" />
                    </div>
                    <div>
                      <label className="block text-gray-700 text-xs font-bold mb-1">CUIT (Sin guiones) *</label>
                      <input type="text" name="cuit" value={formData.cuit} onChange={handleChange} required className="w-full px-3 py-2 text-sm border rounded bg-white" />
                    </div>
                    <div>
                      <label className="block text-gray-700 text-xs font-bold mb-1">Tipo de empresa *</label>
                      <select name="tipoEmpresa" value={formData.tipoEmpresa} onChange={handleChange} required className="w-full px-3 py-2 text-sm border rounded bg-white">
                    ...
                    <div>
                      ...
                      <input type="file" accept=".pdf,.jpg,.png" onChange={(e) => handleFileChange(e, 'constanciaAfip')} required className="w-full text-xs text-gray-500 file:mr-2 file:py-1.5 file:px-3 file:rounded-md file:border-0 file:bg-gray-100 file:text-gray-700 hover:file:bg-gray-200" />
                    </div>
                    ...
                    <div>
                      ...
                      <input type="text" name="repNombre" value={formData.repNombre} onChange={handleChange} required className="w-full px-3 py-2 text-sm border rounded bg-white" />
                    </div>
                    <div>
                      <label className="block text-gray-700 text-xs font-bold mb-1">DNI *</label>
                      <input type="text" name="repDni" value={formData.repDni} onChange={handleChange} required className="w-full px-3 py-2 text-sm border rounded bg-white" />
                    </div>
                    <div>
                      <label className="block text-gray-700 text-xs font-bold mb-1">Teléfono *</label>
                      <input type="text" name="repTelefono" value={formData.repTelefono} onChange={handleChange} required className="w-full px-3 py-2 text-sm border rounded bg-white" />
                    </div>
                    <div>
                      <label className="block text-gray-700 text-xs font-bold mb-1">Carta Aval (PDF)</label>
                      <input type="file" accept=".pdf" onChange={(e) => handleFileChange(e, 'cartaAval')} className="w-full text-xs text-gray-500 file:mr-2 file:py-1.5 file:px-3 file:rounded-md file:border-0 file:bg-gray-100 file:text-gray-700 hover:file:bg-gray-200" />
                    </div>
                  </div>
                )}
            ...
            {/* --- SECCIÓN 3: EQUIPO --- */}
            <section className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
              <button type="button" onClick={() => toggleSection(3)} className="w-full flex justify-between items-center p-4 bg-gray-50 hover:bg-gray-100 transition-colors">
                <h3 className="text-lg font-bold text-brand-teal font-heading"><i className="fas fa-users mr-2"></i> 3. Equipo Emprendedor</h3>
              ...
              <div className={`${openSection === 3 ? 'block' : 'hidden'} p-6 border-t border-gray-200 animate-fade-in`}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="number" name="cantidadColaboradores" value={formData.cantidadColaboradores} onChange={handleChange} required className="w-full px-3 py-2 text-sm border rounded bg-white" min="0" />
                  </div>
                  <div>
                    <label className="block text-gray-700 text-xs font-bold mb-1">Cantidad de socios (toma de decisiones) *</label>
                    <input type="number" name="cantidadSocios" value={formData.cantidadSocios} onChange={handleChange} required className="w-full px-3 py-2 text-sm border rounded bg-white" min="1" />
                  </div>
                  ...
                  <div className="md:col-span-2 mt-4 border-t border-gray-100 pt-4">
                    ...
                    <textarea name="vinculacionesInstitucionales" value={formData.vinculacionesInstitucionales} onChange={handleChange} required rows={2} className="w-full px-3 py-2 text-sm border rounded bg-white resize-none"></textarea>
                  </div>
                </div>
              </div>
            </section>
            ...
            <section className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
              <button type="button" onClick={() => toggleSection(4)} className="w-full flex justify-between items-center p-4 bg-gray-50 hover:bg-gray-100 transition-colors">
                <h3 className="text-lg font-bold text-brand-teal font-heading"><i className="fas fa-lightbulb mr-2"></i> 4. Innovación y Validaciones</h3>
              ...
              <div className={`${openSection === 4 ? 'block' : 'hidden'} p-6 border-t border-gray-200 animate-fade-in`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                  <div>
                    ...
                    <select name="areaInnovacion" value={formData.areaInnovacion} onChange={handleChange} required className="w-full px-3 py-2 text-sm border rounded bg-white">
                      <option value="">Seleccionar...</option>
                  ...
                  <div>
                    ...
                    <select name="gradoInnovacion" value={formData.gradoInnovacion} onChange={handleChange} required className="w-full px-3 py-2 text-sm border rounded bg-white">
                      <option value="">Seleccionar...</option>
                ...
                <div className="border-t border-gray-100 pt-4 mb-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-6 text-sm">
                    <div className="flex justify-between border-b border-gray-50 pb-2">
                      <span className="text-gray-600">¿Problema validado?</span>
                      <select name="valProblema" value={formData.valProblema} onChange={handleChange} className="border rounded bg-white text-xs px-2 py-1"><option value="">--</option><option value="SI">SÍ</option><option value="NO">NO</option></select>
                    </div>
                    <div className="flex justify-between border-b border-gray-50 pb-2">
                      <span className="text-gray-600">¿Segmento definido?</span>
                      <select name="valSegmento" value={formData.valSegmento} onChange={handleChange} className="border rounded bg-white text-xs px-2 py-1"><option value="">--</option><option value="SI">SÍ</option><option value="NO">NO</option></select>
                    </div>
                    <div className="flex justify-between border-b border-gray-50 pb-2">
                      <span className="text-gray-600">¿Solución validada?</span>
                      <select name="valSolucion" value={formData.valSolucion} onChange={handleChange} className="border rounded bg-white text-xs px-2 py-1"><option value="">--</option><option value="SI">SÍ</option><option value="NO">NO</option></select>
                    </div>
                    <div className="flex justify-between border-b border-gray-50 pb-2">
                      <span className="text-gray-600">¿Contás con MVP?</span>
                      <select name="valMVP" value={formData.valMVP} onChange={handleChange} className="border rounded bg-white text-xs px-2 py-1"><option value="">--</option><option value="SI">SÍ</option><option value="NO">NO</option></select>
                    </div>
                  </div>
                </div>
                ...
                <div className="border-t border-gray-100 pt-4">
                  ...
                  <select name="trl" value={formData.trl} onChange={handleChange} required className="w-full px-3 py-2 text-sm border rounded bg-white">
            ...
            <section className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
              <button type="button" onClick={() => toggleSection(5)} className="w-full flex justify-between items-center p-4 bg-gray-50 hover:bg-gray-100 transition-colors">
                <h3 className="text-lg font-bold text-brand-teal font-heading"><i className="fas fa-leaf mr-2"></i> 5. Impacto y Sostenibilidad</h3>
              ...
              <div className={`${openSection === 5 ? 'block' : 'hidden'} p-6 border-t border-gray-200 animate-fade-in`}>
                ...
                <div className="mb-4">
                  ...
                  <div className="flex gap-4">
                    <label className="flex items-center text-sm"><input type="radio" name="esTripleImpacto" value="SI" checked={formData.esTripleImpacto === 'SI'} onChange={handleChange} className="mr-2 text-brand-teal" /> SÍ</label>
                    <label className="flex items-center text-sm"><input type="radio" name="esTripleImpacto" value="NO" checked={formData.esTripleImpacto === 'NO'} onChange={handleChange} className="mr-2 text-brand-teal" /> NO</label>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                  <div className="md:col-span-2">
                    ...
                    <textarea name="impactoEconomico" value={formData.impactoEconomico} onChange={handleChange} required rows={2} className="w-full px-3 py-2 text-sm border rounded bg-white"></textarea>
                  </div>
                  <div className="md:col-span-2">
                    <label className="block text-gray-700 text-xs font-bold mb-1">Impacto Medioambiental *</label>
                    <textarea name="impactoMedioambiental" value={formData.impactoMedioambiental} onChange={handleChange} required rows={2} className="w-full px-3 py-2 text-sm border rounded bg-white"></textarea>
                  </div>
                </div>
              </div>
            </section>
            ...
            <section className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
              <button type="button" onClick={() => toggleSection(6)} className="w-full flex justify-between items-center p-4 bg-gray-50 hover:bg-gray-100 transition-colors">
                <h3 className="text-lg font-bold text-brand-teal font-heading"><i className="fas fa-handshake mr-2"></i> 6. Ecosistema y Necesidades</h3>
              ...
              <div className={`${openSection === 6 ? 'block' : 'hidden'} p-6 border-t border-gray-200 animate-fade-in`}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-gray-600">
                    {[
                      ...
                      <label key={necesidad} className="flex items-start">
                        <input type="checkbox" checked={formData.necesidades.includes(necesidad)} onChange={() => handleArrayChange('necesidades', necesidad)} className="mt-1 mr-2 text-brand-teal rounded" /> 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-gray-100 pt-4">
                  <div>
                    ...
                    <select name="apoyoInstitucional" value={formData.apoyoInstitucional} onChange={handleChange} className="w-full px-3 py-2 text-sm border rounded bg-white">
                  ...
                  <div>
                    ...
                    <select name="recibioFinanciamiento" value={formData.recibioFinanciamiento} onChange={handleChange} className="w-full px-3 py-2 text-sm border rounded bg-white">
                      <option value="">Seleccionar...</option><option value="SI">SÍ</option><option value="NO">NO</option>
                    </select>
                  </div>
                </div>
            ...
            <div className="pt-8 mt-4 border-t border-gray-200">
              ...
              >
                {isLoading ? (
                  <span className="flex items-center"><i className="fas fa-circle-notch fa-spin mr-2"></i> Procesando Proyecto...</span>
                ) : (
                  <span className="flex items-center"><i className="fas fa-save mr-2"></i> Registrar Proyecto en la Red</span>
                )}
              </button>
              <p className="text-center mt-4 text-xs text-gray-400 pb-4">
                Al hacer clic en Registrar, confirmas que la información brindada es fidedigna.
              </p>
            </div>
